...
parser.add_argument('--model', '-a', metavar='MODEL', default='vgg_cifar10_ternary',
                    choices=model_names,
                    help='model architecture: ' +
                    ' | '.join(model_names) +
                    ' (default: alexnet)')

parser.add_argument('--results_dir', metavar='RESULTS_DIR', default='./results/ternary',
                    help='results dir')
...
parser.add_argument('--regularize_type', default='sine', type=str, metavar='Regularizor Type',
                ...
                if hasattr(p,'ternary'):
                    ...
                    if hasattr(p,'grad') and 'weight' in params_name_collect[p_id]:
                        ...
                        if args.regularize_type == 'l2':
                            p.grad = p.grad + 2*p.data*weight_decay
                        if args.regularize_type == 'poly':
                            p.grad = p.grad + 4*(2*p.data - 4*(p.data**3))*weight_decay
                        if args.regularize_type == 'sine':
                            p.grad = p.grad + 2*math.pi*torch.sin(math.pi*p.data)*torch.cos(math.pi*p.data)*weight_decay
                        if args.regularize_type == 'sine_grad':
                            regularizer = torch.where((p.data>=-0.5)&(p.data<=0.5), (math.pi/2)*torch.sin(math.pi*p.data), -(math.pi/2)*torch.sin(math.pi*p.data))
                            p.grad = p.grad + regularizer*weight_decay

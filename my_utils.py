...
def save_checkpoint(state, is_best_val=False, is_best_test=False, path='.', 
                    filename='checkpoint.pth.tar', save_all=False):
    filename = os.path.join(path, filename)
    torch.save(state, filename)
    if is_best_val:
...
#def adjust_optimizer_cosine(optimizer, epoch, num_epochs, config):
#    def modifiy_optimizer(optimizer, setting):
#        if 'optimizer' in setting:
#            optimizer = __optimizers[setting['optimizer']](
#                    optimizer.param_groups)
#            logging.debug('OPTIMIZER - setting method = %s' % 
...
#            for key in param_group.keys():
#                if key in setting:
#                    logging.debuy('OPTIMIZER - setting %s = %s' % 
#                                 (key, setting[key]))
#                    param_group[key]
#        return
#    
